...
function TranslateCompletionItems(items, line, expand = false, rangeOffset = 0) {
    return items.map((item) => {
        const result = item;
        const range = new vscode_1.Range(new vscode_1.Position(line.lineNumber, result.textEdit.range.start.character + rangeOffset), new vscode_1.Position(line.lineNumber, result.textEdit.range.end.character + rangeOffset));
